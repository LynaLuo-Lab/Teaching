...
###### MCMC sampling #############
def _calc_MFPT_vec(Q,verbose=False,icount=-1):
	Q_hat = Q[:-1,:-1]
	if verbose and (icount > 0):
		print(
			'counter: ',icount,
			'; # nonzero Qhat eigval: ',
			np.nonzero(np.linalg.eig(Q_hat)[0])[0].shape
		)
...
def monte_carlo_milestoning_error(
	...
	for counter in loopIterator:
		...
		if verbose: 
			print("MCMC stepnum: ", counter,
					'; # eigvecs:',
					np.nonzero(np.linalg.eig(Qnew)[0])[0].shape)
		if counter > skip and counter % stride == 0:
				Q_mats.append(copy.deepcopy(Qnew))
				T_err = _calc_MFPT_vec(Qnew,verbose=True,icount=counter)
